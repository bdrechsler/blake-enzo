                    ...
                    float rho_MII, float rho_AII, float cond[3])
{
  ...
  // collisions with e-
  coeff_sH = sqrtT * (2.841 + 0.093 * theta + 0.245 * pow(theta, 2) - 0.089 * pow(theta, 3));
  coeff_sHe = sqrtT * 0.428;
  coeff_sH2 = sqrtT * (0.535 + 0.203 * theta - 0.163 * pow(theta, 2) + 0.050 * pow(theta, 3));
  // momentum exchange
  ...
  // collisions with H+
  coeff_sH = 0.649 * pow(T, 0.375);
  coeff_sHe = 1.424 + 7.438e-6 * T - 6.734e-9 * pow(T, 2);
  coeff_sH2 = 1.003 + 0.05 * theta + 0.136 * pow(theta, 2) - 0.014 * pow(theta, 3);
  tau_sH = (me + mh) / (rho_HI * coeff_sH);
  tau_sHe = (me + 4.0 * mh) / (rho_HeI * coeff_sH);
  tau_sH2 = (me + 2.0 * mh) / (rho_H2I * coeff_sH);
  ...
  // collisions with He+
  coeff_sH = 4.71e-1;
  coeff_sHe = 2.0 * 8.73e-2 * pow(1.0 - 0.093 * theta, 2);
  ...
  // collisions with H3+
  coeff_sH2 = 2.693 - 1.238 * theta + 0.663 * pow(theta, 2) - 0.089 * pow(theta, 3);
  ...
  coeff_sH2 = sqrtT * (1.476 - 1.409 * theta + 0.555 * pow(theta, 2) - 0.0775 * pow(theta, 3));
  // Langevin approximation H2 -> H
  ...
  coeff_sH = 1.983 + 0.425 * theta - 0.431 * pow(theta, 2) + 0.114 * pow(theta, 3);
  // Langevin approximation H -> H2
